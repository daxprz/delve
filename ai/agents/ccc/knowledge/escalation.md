# Escalation rubric — `ccc` (delve) ↔ central `expert`

> When does the per-project `ccc` (delve's Σ) handle
...
`/bug` and `/feature-request` auto-route through delve's
...
- **Project-internal questions** ("which engineer owns module X?") —
  those go to delve's own liaison or directly to the relevant
...
unambiguously, file a note in delve's `effort/` directory
