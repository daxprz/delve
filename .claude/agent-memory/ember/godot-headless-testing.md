...
7. **Two-process multiplayer tests: the HOST must outlive the client.** If the server quits first, its teardown despawns all replicated nodes on the client mid-assertion (`previously freed` spam). Pattern: client finishes and quits; host treats `get_peers().is_empty()` as end-of-test. Guard cross-peer node refs with `is_instance_valid()`. See `scripts/run_mp_test.sh`.
